...
const Socialproof = () => {
  ...
  return (
    <div className="w-full border-b border-primary/10 flex flex-col justify-center items-center">
      ...
      {/* Logo Grid */}
      <div className="self-stretch border-primary/10 flex justify-center items-start border-t border-b-0">
        <Pattern length={50} />
        ...
        <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 md:grid-cols-4 gap-0">
          {logos.map((_, index) => {
            return (
              <div
                key={index}
                className="group h-28 sm:h-32 md:h-36 flex flex-col sm:flex-row justify-center items-center gap-2 border-l border-r border-b border-primary/10 "
              >
                <div className="w-6 h-6 relative rounded-full">
                  <Workflow className="group-hover:rotate-180 transition delay-300 duration-300 ease-in-out " />
                </div>
